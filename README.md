...
4. Commit and push `.github/workflows/ci-cd.yml` with the application changes. No GitHub starter template is needed. Once pushed to `main`, **CI / CD** runs the backend and frontend checks, followed by deployment. The deployment job reports a configuration error if the VPS settings below are missing.
5. In the GitHub repository, open **Settings → Environments → New environment** and create `staging`. In that environment, under **Environment secrets**, add `VPS_SSH_KEY` with the complete private key generated above, including its BEGIN/END lines, and `VPS_KNOWN_HOSTS` with the VM's verified SSH host-key line. Add `VPS_HOST=84.12.83.196`, `VPS_USER=eventeveryday-deploy`, and `VPS_DEPLOY_PATH=/opt/apps/EventEveryday` as environment variables. Verify the host-key fingerprint out of band before saving it; do not obtain it blindly during deployment. Do not paste the private key into issues, commits, or chat. Set environment protection rules if deployment should require review.
6. Pushes to `main` run the deploy job against the `staging` environment. If any variable or secret is missing, its configuration check fails visibly. From **Actions → CI / CD**, use **Run workflow** on `main` for the first deployment if no new push is pending.
