...
6. Open **Settings → Secrets and variables → Actions → Variables** and add repository variables `VPS_HOST=84.12.83.196`, `VPS_USER=eventeveryday-deploy`, and `VPS_DEPLOY_PATH=/opt/apps/EventEveryday`. Add these after the environment secrets. Pushes to `main` then run the deploy job; if any variable or secret is missing, its configuration check fails visibly instead of leaving a successful CI run with deployment skipped. From **Actions → CI / CD**, use **Run workflow** on `main` for the first deployment if no new push is pending.
